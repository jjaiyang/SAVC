...
#         self.model = nn.DataParallel(self.model, list(range(self.args.num_gpu)))
    ...
    def get_optimizer_base(self):
        ...
        optimizer = torch.optim.AdamW(self.model.parameters(), self.args.lr_base, momentum=0.9, nesterov=True,
...
'epoch:%03d,lr:%.4f,training_loss:%.5f,joint_loss:%.5f, moco_loss:%.5f, moco_loss_global:%.5f, moco_loss_small:%.5f,training_acc:%.5f,test_loss:%.5f,test_acc:%.5f'% (epoch, lrc, tl, tl_joint, tl_moco, tl_moco_global, tl_moco_small, ta, tsl, tsa))    
                ...
                if not args.not_data_init:
                    ...
                    self.model = replace_base_fc(train_set, testloader.dataset.transform, self.transform, self.model, args) # 基类训练结束后更新旧类fc对应权重为平均向量
                ...
                self.model.update_fc(trainloader, np.unique(train_set.targets), self.transform, session) # 在finetuning前直接更新新类在model.fc上的平均向量（为什么前：样本少）;同时更新了new_fc用于update_fc_ft()中
                if args.incft:
                    trainloader.dataset.transform = train_transform
                    train_set.multi_train = True
                    update_fc_ft(trainloader, self.transform, self.model, self.num_trans, session, args) # 新类训练
